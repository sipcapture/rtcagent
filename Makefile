...
.check_%:
	@command -v $* >/dev/null
	if [ $$? -ne 0 ]; then
		echo "RtcAgent Makefile: missing required tool $*"
		exit 1
	else
...
env:
	@echo ---------------------------------------
	@echo "RtcAgent Makefile Environment:"
	@echo ---------------------------------------
...
RTCAGENT_NAME = $(shell $(CMD_GREP) "Name:" builder/rpmBuild.spec | $(CMD_AWK) '{print $$2}')
RPM_SOURCE0 = $(RTCAGENT_NAME)-$(RTCAGENT_VERSION).tar.gz
...
help:
	...
	@echo ""
	@echo "# build"
	@echo "    $$ make all					# build rtcagent"
	@echo ""
	@echo "# build rpm"
	@echo "    $$ make rpm VERSION=0.0.0 RELEASE=1		# build rtcagent rpm"
	@echo ""
	@echo "# clean"
