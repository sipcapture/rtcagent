...
type ReceiveInfoOpensips struct {
	SrcIP   IpAddr
	DstIP   IpAddr
...
func (kem *OpensipsEvent) PayloadLen() int {
	return int(kem.DataLen)
}

func (kem *OpensipsEvent) StringHex() string {
...
func (kem *OpensipsEvent) String() string {
	//addr := kem.module.(*module.MOpenSSLProbe).GetConn(kem.Pid, kem.Fd)
...
func (kem *OpensipsEvent) SendHep() bool {
	//Lets allow to send HEP
	return true
}

func (kem *OpensipsEvent) GenerateHEP() ([]byte, error) {
...
func (kem *OpensipsEvent) Clone() IEventStruct {
	event := new(OpensipsEvent)
